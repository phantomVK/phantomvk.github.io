...
在网上搜索相关主题，大家会发现答案千篇一律地使用9-Patch图片作为TextView背景。这个方法我也用过，效果不好不说，还存在不少问题。

使用9-Patch文件，本质是用图片作为背景，只是这张图片会在设置的某些像素点上重复绘制达到拉伸的效果。因为Android屏幕碎片化问题，所以一张9-Patch不能适配所有屏幕，至少需要两张分别适配 __xhdpi__ 和 __xxhdpi__。

虽然每张9-Patch体积不大，但扩展到 __不同场景要求不同圆角__、__点击气泡时背景交互色变化__、__更加精细的屏幕适配__，最终需要多款类似而有各自差别的图片集，累积成可观的安装包大小。其次用9-Patch作为消息气泡，气泡描边的视觉效果相当糟糕，在实战中已得到充分验证。

效果最好，莫过于通过代码实现消息背景。由于通过xml的 __Shape__ 样式难以绘制箭头，仅能实现描边和填充颜色，所以还得通过代码实现。
...
3. __arrowMarginTop__ 是箭头上角距离(左、右)上方圆角的垂直高度；
...
由于宽度和气泡内部 __TextView__ 文字长度高度有关，所以需要重写方法实时计算宽高值。此方法中计算 __气泡上部path__ 和 __气泡下部path__ 。此外还有气泡中部，不过中部纯粹为一个的矩形，计算好高度直接绘制即可。
