...
首先获取`SharedPreferences`实例，调用`sp.edit()`获得可编辑实例，写入数据并提交。
...
`SharedPreferences`是一个接口，源码在`/frameworks/base/core/java/android/content`
...
public interface SharedPreferences {
    ...
    public interface OnSharedPreferenceChangeListener {
        // 当SharedPreference发生修改、新增、移除时回调，key为变化的键，并执行在主线程
        // @param sharedPreferences 发生变化的SharedPreferences
        // @param key 发生修改、新增、移除的键
    ...
    // 通过key获取字符串集合，且一定不能修改返回的数据，否则会导致数据不一致的问题
    @Nullable
