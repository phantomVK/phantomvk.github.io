...
这篇文章将深入介绍`CAS`在`JDK`中，填补[Java源码系列(7) -- AtomicInteger](https://phantomvk.github.io/2018/01/17/AtomicInteger/)里面`Unsafe`和`CAS`内容的空缺。从高层调用开始，经历JDK、JNI和asm汇编，最终调用处理器CAS指令集，带你浏览整个实现流程。
...
下面unsafe.cpp大概在尾部有个保存方法对应关系的静态数组：
...
其执行流程：
 ...
 - 根据成员变量`value`反射后计算出的内存偏移值`offset`，在内存中取指针`addr`
...
`cmpxchg`方法签名：
 ...
 - `exchange_value`对应更新值`x`
 - `dest`对应指针`addr`
 - `compare_value`对应期待值`e`
...
这个结构告诉编译器层添加一个内存障碍，详见[参考链接-3](https://stackoverflow.com/questions/14950614/working-of-asm-volatile-memory)：
...
有人提议[参考链接-12](https://bugs.java.com/view_bug.do?bug_id=8185062)把`os::is_MP()`的`AssumeMP`永久设为`true`提升性能。且在2017-10-03的JDK10b13实现该提议，所以上述伪代码可以进一步演进为：
