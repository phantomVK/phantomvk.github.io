...
每次被操作元素都会被移到队列末位。一段时间后本来处于队列末位的元素，因为其他元素的操作逐渐前进到队头。如果队列空间足够，所有元素都不会移除。否则处于队头的元素优先移除，腾出空间容纳新元素。所以使用热度高的资源得到有效缓存，长时间没有使用的资源会被移出队列。这些操作均依赖LinkedHashMap，深入了解请阅读文章：[Java源码系列(11) -- LinkedHashMap](https://phantomvk.github.io/2018/07/09/LinkedHashMap/)
...
public final V put(K key, V value) {
    ...
    synchronized (this) {
        ...
        previous = map.put(key, value // 被替换出来的元素
        // 需要减去前一个元素空间占用
        if (previous != null) {
    ...
    if (previous != null) {
        entryRemoved(false, key, previous, value); // 执行移除操作
    }

    trimToSize(maxSize); // 调整大小
    return previous;
}
```
...
public void trimToSize(int maxSize) {
    while (true) {
        ...
        synchronized (this) {
            ...
            map.remove(key); // 从LinkedHashMap中移除
            size -= safeSizeOf(key, value); // 调整总大小
            evictionCount++;
        }
...
在子类选择性重写此方法，目的是键值命中失败时提供自定义创建功能，把新建item加入队列，默认返回null。
...
私有方法，对`sizeOf()`方法返回结果安全检查。假设item中存放的是Bitmap对象，需重写sizeof对象实现获取该对象的内存大小。为什么在已预设队列长度的情况下还要重写此方法？
...
拿图片缓存为例：虽然我们可以控制图片在队列中缓存的数量，但却无法控制所有图片在内存中的占用。可能出现队列保存若干张图，每张图片占用多达16M内存，以至于为数不多的图片占用大量内存。为此，折中的解决方案是计算图片内存占用，即使item依然保存在队列中，当内存占用超过阈值，也要强制释放队列最近最少使用资源，被释放图片以后使用时再重新加载。
