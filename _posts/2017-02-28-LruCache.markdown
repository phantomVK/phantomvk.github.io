...
`LruCache`通过`LinkedHashMap`数据结构来完成item的保存，传送门：[LinkedHashMap源码阅读](https://phantomvk.github.io/2018/07/09/LinkedHashMap/)。
...
用来保存各种操作统计次数，如：每次添加元素`putCount`递增。这些统计可以作为性能优化的参考值。
...
参数`maxSize`记录整个列表最大保存数。为避免严重的哈希冲突，`LinkedHashMap`哈希因子设定为0.75。

`LinkedHashMap`构造函数参数`accessOrder`为`true`时以访问顺序排列元素，否则以插入顺序排列元素为准。从源码可知`LruCache`使用`LinkedHashMap`时`accessOrder`为`true`。
...
可以修改列表最大保存数量，如果maxSize变小，则需要通过`trimToSize()`移除多余元素。
...
有些类型的实例拥有独特的对象回收逻辑，当实例被移出队列时，应该通过此(需重写)方法完成销毁操作。否则这个被移除的对象仅按照虚拟机垃圾回收策略进行回收。
...
为此，折中的解决方案是计算图片内存占用，即使item依然保存在队列中，当内存占用超过阈值，也要强制释放队列最近最少使用的资源。当被释放图片以后再次使用时，可通过create(K key)重新加载。
