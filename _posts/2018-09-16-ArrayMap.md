...
ArrayMap是通用的键值对映射数据结构，相比传统HashMap有更高内存利用率，使用数组结构保存映射：一个整形数组存放哈希值，一个Object数组存放键值对。使用数组能避免为每个放入Map的entry创建额外对象(对比[HashMap#Node](https://phantomvk.github.io/2018/06/30/HashMap/#51-%E6%A1%B6%E8%8A%82%E7%82%B9))，并能更有力地控制数组扩展的大小(扩增数组需创建一个新数组并拷贝元素，不重建hash map)。

```java
public final class ArrayMap<K, V> implements Map<K, V>
```

相比传统HashMap，ArrayMap速度更慢且不需要存放大量items的场景。因为ArrayMap通过二分查找搜索item，添加或移除元素需要操作数组的entries。容器仅持有数百个items时，两者性能差距不大。
...
// 返回包含ArrayMap所有键值对的集合
@Override
public Set<Map.Entry<K, V>> entrySet() {
    return getCollection().getEntrySet();
}

// 返回包含ArrayMap所有key的集合
@Override
public Set<K> keySet() {
    return getCollection().getKeySet();
}

// 返回包含ArrayMap所有value的集合
@Override
