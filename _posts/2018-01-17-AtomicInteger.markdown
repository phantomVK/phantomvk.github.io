...
说到CAS，需要了解的是：这个能力并非由操作系统或JVM提供，而是CPU原生支持的。如果一个CPU不能保证CAS能力，那这个CPU的完全没有数据修改安全可言。

如在JDK源码中，就有一段代码调用处理器CAS指令`"cmpxchgl %1,(%3)"`。

说回`AtomicInteger`，这类不能看见底层的运作，因为主要调用了`Unsafe`才实现CAS。
