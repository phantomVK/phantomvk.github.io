...
没有优化前启动，33秒突破32MB内存分配，并在40秒到50秒期间出现多次接近64MB的内存尖峰和垃圾回收

![dump_size_compare](/img/android/performance/dump_size_compare.png)

对比优化后，在28秒轻微越过32MB堆内存，而且随后内存申请曲线相对柔和，启动完成时间从55秒提前到35秒。随后也不再触发垃圾回收操作，所以内存维持在稍高水平

1分钟整均完成启动停在主界面，把应用退到后台，两者堆内存减少到21MB，可见应用后台占用内存不多

### 五、总结

此次优化证明处理方案可减少启动过程对象产生数量，缩减最大内存占用量。若考虑用户动态使用场景、隐含未知的内存泄漏、图中尚存在的尖峰，后续还有进一步优化空间
