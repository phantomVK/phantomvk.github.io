...
* 线程私有数据区：
  * **虚拟机栈**：执行方法时所需栈帧，存储局部变量、操作数栈、动态链接、方法出口等信息；
...
方法执行完毕栈帧出栈并销毁(移动内存指针)，然后继续执行虚拟机栈顶的栈帧。
...
从分配的角度来看，线程本地缓冲区(Thread local allocation buffer, TLAB)有利于高效地划分线程私有缓冲区，避免每次线程需要内存时都请求堆区开辟新空间。
