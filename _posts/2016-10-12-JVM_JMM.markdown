...
程序计数器可看作当前线程所执行字节码行号的指针，每个线程有独立的程序计数器，线程之间计数器互不影响。执行Java方法时，这个计数器记录执行字节码指令地址。如果当前线程执行的是Native方法，则计数器为空。
...
堆区是开发过程中工程师接触次数最多、最受关注的内存区域。该区域被所有线程共享，所有对象都在这个区域分配内存并初始化，内存分配及回收操作消耗非常多运算资源。相比之下，栈更容易管理且轻巧，所有内存都在进出栈过程中自然而然地完成申请和释放。
...
新创建对象先存放在Eden区，经过第一次垃圾回收且存活的对象会进入两个Survivor中的一个。此时这个Survivor区就称为 __To Survivor__ ，而另一个区称为 __From Survivor__ 。From Survivor把上次回收存活的对象移到To Survivor并清空。
...
如果一个新对象体积太大，以至于新生代经过一次垃圾回收后依然没有足够空间存放它。JVM会通过分配担保的方式把这个对象放在老年代。如果老年代经过一次Full GC依然没有空间，虚拟机无法为这个对象提供内存空间，只能抛出OOM错误。
...
基于G1回收的特殊性，G1的数据区模型和上面介绍的模型是有差别的。也就是说，上面的模型不能用在G1上。现时没有太多资料供参考G1数据区模型，如果没有特殊需求，现时CMS和PerNew已经很好地适应大部分应用场景。
