...
新创建对象先存放在Eden区，经过第一次垃圾回收且存活的对象会进入两个Survivor中的一个。此时这个Survivor区称为 __To Survivor__ ，另一个区称为 __From Survivor__ 。把上次回收存活的对象从From Survivor移到To Survivor并清空。
...
在Survivor区的对象会在两区之间来回经历GC。经过多次垃圾回收(CMS, G1都是15)依然存活的对象，可以假定此对象比较稳定，达到GC年龄后移到老年代，此后在老年代中经历垃圾回收的频率将大大降低。
...
基于G1回收的特殊性，G1的数据区模型和上面介绍的模型是有差别的。进一步说，上述模型不能用在G1上。如果没有特殊需求，例如堆空间超过4G，现时CMS和PerNew已经很好地适应大部分应用场景。
