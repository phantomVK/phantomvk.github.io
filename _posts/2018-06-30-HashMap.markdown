...
- 位运算为`0`的放在原桶索引位置`oldIndex`
- 位运算为`1`的放在新桶索引位置`oldIndex+oldCap`
...
查找 __value__ 不像 __key__：

- __key__ 能先算出其hash值并跳到对应桶进行查找；
- 而 __value__ 只能遍历整张表所有元素逐一对比；
...
- 哈希表不为空，返回哈希表长度；
- 哈希表为空但`threshold`大于0，返回`threshold`；
- 否则返回默认初始化大小16
