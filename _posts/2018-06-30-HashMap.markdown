...
HashMap是软件工程师使用频率高、用来保存键值对映射的容器类。自JDK8始，HashMap引入红黑树，调整优化哈希算法，性能相比JDK7有进一步提升。同时对JDK7中存在多线程操作导致死循环的问题进行了优化。

本文基于JDK10的源码进行介绍，代码格式进行轻微调整以便阅读，最终逻辑与JDK8版本无异。由于HashMap源码较多避免篇幅过长，部分内容不做介绍，请自行查阅。

## 二、类签名

假设键值对能正确分散到不用桶中，那么访问get()和put()方法的时间消耗可近似为一个时间常数。哈希表遍历性能与哈希桶(table长度)、Node节点(键值对)数量成比例，因此不合适把初始capacity设置太大，或负载因子值load-factor(默认0.75)设置过小。
...
作为通用规范，默认负载因子值0.75可以很好的平衡`时间开销`和`内存开销`之间的关系。高负载因子值(如>0.75)有助于减少内存空间开支，但会增加查找的时间开销。如果在构建的时候正确指定哈希表初始容量，可以完全避免以后重哈希的操作，换句话说就是令插入操作更高效。
...
// 下次表扩容阀值，计算自： threshold = capacity * load factor
int threshold;
...
`Node<K,V>[] table`实现`Map.Entry<K,V>`示意UML：
...
为减低碰撞几率，需设置负载因子 load factor（默认值0.75）。假设用默认桶数量capacity（初始值16），通过`threshold = capacity * load factor`可知当键值对数量超过12时触发扩容。当然，通过单纯使用大量的桶，即便不经优化的哈希算法，也能把元素相对均匀地放在不同桶中，类似空间换时间。事实上，JDK中的HashMap经过长期验证和修改，已能很好地在空间和时间两者取得平衡。

过往的HashMap在桶数量较少时，连续值经过哈希运算很容易被分配到相同的桶，从而导致哈希碰撞。JDK8把哈希值`高16位`与`低16位`进行亦或运算。在不降低性能的前提下充分利用有限的条件提高混淆度，尽力把元素分配到不用桶。
