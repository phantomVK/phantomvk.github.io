...
为降低碰撞几率，需设置负载因子 loadFactor(默认值0.75)。假设用默认桶数量capacity(初始值16)，通过`threshold = capacity * loadFactor`可知当键值对数量超过12时触发扩容。当然，通过单纯使用大量桶，即便不经优化的哈希算法，也能把元素相对均匀地放在不同桶中，类似通过空间换时间。事实上，JDK中的HashMap经过长期验证和修改，已在空间和时间两者取得平衡。
...
从图中看出，当流程进行到`n|=n>>>4`，后续步骤运算结果已经固定不变。参数cap为15计算得到最终结果为16。
